...
<Accordion class="my-5">
	<AccordionItem>
		...
		<div class="flex justify-end">
			<Button onclick={() => (ruleSet.rule = RuleType.simple)}>submit</Button>
		</div>
	</AccordionItem>
	<AccordionItem>
		...
		<div class="flex items-center gap-3 m-3">
			<p>正解ポイント</p>
			<input type="number" style="width:4rem" bind:value={ruleSet.whenCorrect} />
			<p>誤答ポイント</p>
			<input type="number" style="width:4rem" bind:value={ruleSet.whenIncorrect} />
		</div>
		<div class="flex justify-end">
			<Button
				onclick={() => {
					ruleSet.rule = RuleType.mn;
				}}
			>
				submit
			</Button>
	...
	<AccordionItem>
		...
		<div class="flex justify-end">
			<Button onclick={() => (ruleSet.rule = RuleType.updown)}>submit</Button>
		</div>
	</AccordionItem>
	<AccordionItem>
		...
		<div class="flex justify-end">
			<Button onclick={() => (ruleSet.rule = RuleType.swedish)}>submit</Button>
		</div>
	</AccordionItem>
	<AccordionItem>
		...
		<div class="flex items-center gap-3 m-3">
			<p>初期ポイント</p>
			<input type="number" min="1" style="width:4rem" bind:value={ruleSet.inicialPoint} />
		</div>
		<div class="flex justify-end">
			<Button
				onclick={() => {
					ruleSet.rule = RuleType.divide;
				}}
			>
				submit
			</Button>
	...
	<AccordionItem>
		...
		<div class="flex items-center gap-3 m-3">
			<p>誤答時倍率</p>
			<input type="number" max="-1" style="width:4rem" bind:value={ruleSet.whenIncorrect} />
		</div>
		<div class="flex justify-end">
			<Button
				onclick={() => {
					ruleSet.rule = RuleType.backstream;
					ruleSet.whenCorrect = 1;
				}}
			>
				submit
			</Button>
